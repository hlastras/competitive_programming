	...
	for y := 0; y < len(matrix); y++ {
		for x := 0; x < len(matrix[0]); x++ {
			...
			for x2 := x + 1; x2 < len(matrix[0]); x2++ {
				...
				if _, ok := steps[id2]; ok && valid(matrix[y][x2]) {
					intersectionsXR++
				}
			}

			fmt.Println(y+1, x+1, intersectionsXR)
			...
			//	if _, ok := steps[id2]; ok && valid(matrix[y2][x]) {
			//		intersectionsYU++
			//	}
			//}
			//
			...
			//}
			//
			//fmt.Println(y+1, x+1, par, impar, impar == par, intersectionsXR, intersectionsXL, intersectionsYU, intersectionsYD)
			//values := []int{}
...
func valid(u uint8) bool {
	if u == '7' || u == 'F' || u == 'L' || u == 'J' || u == 'S' || u == '|' {
		//if u == '7' || u == 'F' || u == 'L' || u == 'J' || u == '-' || u == '|' {
		//if u == '-' || u == '|' || u == 'J' || u == 'F' {
		return true
	}
	return false
